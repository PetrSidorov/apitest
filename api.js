 ...
 */
 class Api
 {
     ...
     {
        let result = '';
        const replacedID = template.replace('%id%', object.id);
        const replacedName = replacedID.replace('%name%', encodeURIComponent(object.name));
        const replacedRole = replacedName.replace('%role%', encodeURIComponent(object.role));
        const replacedSalary = replacedRole.replace('%salary%', object.salary);
        result = replacedSalary
        return result;
     }
 }
